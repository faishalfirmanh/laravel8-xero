...
use App\Http\Repository\MasterData\BusinessRepository;
...
class BusinessLineController extends Controller {
      ...
      public function getAllPaginate(Request $request)
    {
        ...
        if ($request->keyword != null) {
            $data = $this->repo->searchData($where, $request->limit, $request->page, 'name_business', strtoupper($request->keyword));
        } else {
            $data = $this->repo->getAllDataWithDefault($where, $request->limit, $request->page, 'name_business', 'ASC');//getDataPaginate("name",10,$request->keyword);
        }
    ...
    public function destroy(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'id' => 'required|numeric|exists:business_lines,id',
        ]);
